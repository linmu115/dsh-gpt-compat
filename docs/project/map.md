...
**当前基线：0.5.0-dev.8，包含原生压缩轨迹；Maintenance 使用扩展数据 Adapter。** 推理强度由模型能力声明。长任务与成本评估仍待验收，主实例未切换。CPA 账号模块新增本机启动入口，本次安装和真实按钮验收状态见 [[VER-cpa-start]]，开发过程见 [[HIST-cpa-start]]。
