...
**当前基线：0.5.0-dev.8，包含原生压缩轨迹；Maintenance 使用扩展数据 Adapter。** 推理强度由模型能力声明。长任务与成本评估仍待验收，主实例未切换。CPA 账号模块 0.1.3 的本机启动入口已在副本真实验收，安装和故障恢复边界见 [[VER-cpa-start]]，开发过程见 [[HIST-cpa-start]]。

## 已经确定的行为

- 本插件的 Responses 提供商自动覆盖 GPT 会话模型；CPA 连接后自动发现型号、容量和推理档位。设置页只读展示，选择留在会话页，每个会话在安全步骤边界切换。其它提供商沿用高级显式绑定。
...
- [[VER-model-catalog|自动模型目录验证]]：设置只读展示、CPA 型号与推理能力发现、会话菜单和副本安装检查；不代表逐型号真实生成验收。

- [[HIST-model-discovery|模型目录改造历程]]：从需求澄清到加载修复、菜单验证与地图补录；经验可在“开发历程 → 按问题查找”中按类别浏览。
