...
0.5.0-dev.2 修复经典脚本 ModuleLoader 注册，dev.3 补齐模型推理配置。当前源码 64 项离线测试通过，1 项可选 live 测试跳过，类型检查通过；真实账号结果属于之前明确记录的验收。
...
2026-09-17 用户澄清：GPT 设置区不需要模型下拉选择或任何模型/提供商填写，模型选择留在会话页。现改成自动检测并只读展示全部 GPT 会话模型。CPA 账号模块读取账号支持的模型和元数据，原生 Responses 自动提供模型容量及推理档位；原先三型号的手填范围不再限制本插件提供商。详情与本次验收见 [设置页记录](../../../settings-ui.md)。
