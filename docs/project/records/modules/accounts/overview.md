...
2026-09-18 新增明确需求：CPA 停止时，在此设置区显示服务状态并提供“启动本机 CPA”。后端只启动部署者配置的本机程序，等待实际响应，合并重复点击；已运行不重复启动，失败展示原因。不得借启动改动账号选择或登录凭据。验收包含未运行、已运行、启动失败和页面反馈。当前实现与部署结果见后续本次验证记录。
