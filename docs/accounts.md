...
部署者在账号模块的后端配置 `localLaunch`：`executable` 是 CPA `.exe` 的绝对路径，`configFile` 是其配置文件，`passwordFile` 是本机管理密码文件。配置不通过页面提交；密码内容不进入启动命令。默认未配置时禁止启动。请确认这三个文件在实际 DSH 宿主进程中可访问；独立终端能读取不等于宿主可读取。文件检查失败时，0.1.3 会在宿主日志输出去重的错误码及路径指纹，不输出密码内容或原路径。凭据文件应保留私有权限并排除版本控制。目前只支持 Windows；通过无 shell、隐藏窗口、分离进程启动，沿用现有账号和路由配置。
