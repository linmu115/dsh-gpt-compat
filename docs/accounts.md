...
入口为“设置 → GPT 适配 → CPA 账号”，沿用 GPT 兼容工具的 DSH 原生按钮、输入框、卡片与主题颜色。首次连接后，从账号卡片点击“设为代理账号”，确认后才启用固定代理。新增账号保留待用；移除当前账号会先暂停代理；移除待用账号不改变当前选择。原有路由状态不伪装成已经固定某个账号。
...
“本机 CPA 服务”卡片读取运行状态，停止时可点击“启动本机 CPA”。启动过程中禁止重复点击，等待 CPA 返回服务标识后才显示已运行，并刷新账号与模型目录。已经运行时不会创建第二个进程。服务就绪与管理密码有效是两件事：密码错误会单独提示，不会被显示成启动失败或剩余额度为零。

部署者在账号模块的后端配置 `localLaunch`：`executable` 是 CPA `.exe` 的绝对路径，`configFile` 是其配置文件，`passwordFile` 是本机管理密码文件。配置不通过页面提交；密码内容不进入启动命令。默认未配置时禁止启动。目前只支持 Windows；通过无 shell、隐藏窗口、分离进程启动，沿用现有账号和路由配置。

认证接口增加只接收 action 的 `localStatus` 和 `localStart`，拒绝客户端附加路径或命令。检测使用 loopback TCP 和最多 16 KiB 的 CPA 根响应；非 CPA 端点不覆盖启动。跨实例使用可执行文件旁的独占启动锁，并检查同路径进程。锁冲突或就绪超时明确报错，不强杀进程、不自动删遗留锁；部署者需核对进程和日志后处理。相同路径的进程即使使用其他配置也不会重复启动，必要时由部署者协调实例。
