...
declare module '@deepseek-ai/cordis' {
  interface Context { gptCpaAccounts: { identity(endpoint: string): Promise<string | undefined> } }
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.provide('gptCpaAccounts', { identity: endpoint => service.identity(endpoint) })
  ctx.effect(() => ctx.connection.fetch.register({
    path: '/api/gpt-compat.accounts', methods: ['POST'], requestBody: 'buffered',
    fetch: request => handleAccounts(service, request),
  }), 'GPT account management')
}

/** The Connection service supplies authentication and Host/Origin validation. */
export async function handleAccounts(service: CpaAccounts, request: Request): Promise<Response> {
